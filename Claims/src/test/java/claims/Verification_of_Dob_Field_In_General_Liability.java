...
public class Verification_of_Dob_Field_In_General_Liability extends BaseClass{
	
	@Test
	public void dobFieldforGeneralLiabilityClaim() throws InterruptedException {
...
for (;;) {
	try {
		driver.findElement(By.xpath("//a[text()='" + claimentName + "']/../../..//i[@title='Claims Details']")).click();
		break;
	} catch (Exception e) {
...
softassert.assertEquals(dobonNewClaimPaage, dobTBonDetailsPage,"DOB is not matching in Claim Details Page for General Liability Claim");
...
WebElement updationMsg = driver.findElement(By.xpath("//div[@class='bootstrap-growl alert alert-success alert-dismissible']"));
explicit.until(ExpectedConditions.visibilityOf(updationMsg));
extentTest.info(driver.findElement(By.xpath("//div[@class='bootstrap-growl alert alert-success alert-dismissible']")).getText());

String dobAfterUpdate = driver.findElement(By.id("DOB")).getAttribute("value");

softassert.assertEquals(dobAfterUpdate,dobDataonUpdate,"DOB is not matching in Claim Details Page after updating for General Liability Claim");
